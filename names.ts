...
const lsb = (n: number) => Math.floor(n) % 0x7f;
const msb = (n: number) => lsb(Math.floor(n >> 9));
const buckets: number[] = Array(tableSize * 2).fill(unused);
...
const hash = (s: string, factor: number) => {
    let h = 0;
    for (var c of s) {
...
const lookupEntry = (s: string, factor: number) => {
    const h = hash(s, factor);
    const hi = msb(h);
    const lo = lsb(h) * 2;
...
const run2 = (words: string[], factor: number) => {
    maxMissed = 0;
    wordCount = 0;
    missed = 0;
    totalMissedBy = 0;
