...
// const msb = (n: number) => Math.floor(n / 0x100) % 0xff;
const lsb = (n: number) => n % 0x7f;
const msb = (n: number) => Math.floor(n / 0x80) % 0xff;
...
const hash = (s: string, prime: number = 13) => {
    let sum = 0;
...
const addEntry = (key: string, prime: number) => {
    const h = hash(key, prime);
    const lo = lsb(h);
    const hi = msb(h);
    let lo1 = lo;
...
const tableInit = (wordList: string[], prime: number) => {
    for (const w of wordList) {
        const index = addEntry(w, prime);
        if (index === unused) break;
    }
}

const tableLookup = (s: string, prime: number) => {
    const h = hash(s, prime);
    let lo = lsb(h);
    const hi = msb(h);
...
const run = (words: string[], prime: number) => {
    tableInit(words, prime);
...
for (let prime = 1; prime < 0x100; prime++) {
    maxMissed = 0;
    wordCount = 0;
    missed = 0;
    totalMissedBy = 0;
    ...
    buckets.fill(unused);
    run(wordsx, prime);
    const entry = tableLookup('key', prime);
    const avgMissSize = Math.floor(totalMissedBy / wordCount)
    if (avgMissSize <= 1 && wordCount >= 117 && totalMissedBy <= 203) {
        console.log("--------------------------");
        console.log("prime: ", prime);
        console.log('missed ', missed)
        console.log('avg miss ', avgMissSize)
        console.log('word count ', wordCount)
        // console.log('rejections ', rejections)
        // console.log('collisions ', collisions)
        // console.log(entry)
        // console.log('max miss ', maxMissed)
